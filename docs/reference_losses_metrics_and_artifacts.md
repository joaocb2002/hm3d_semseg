...
- **JSON** stores one complete structured value. Files such as `summary.json`
  and `provenance.json` are rewritten atomically as complete documents.
- **JSONL** stores one independent JSON object per line. `metrics.jsonl` is
...
`metrics.jsonl` is the authoritative time series. `metrics_summary.json` is a
human-oriented derived summary of that series.

## Human report versus raw record

Every completed training run automatically creates `report/index.html`. Start
...
Regeneration replaces only derived files below `report/`; it does not alter
`metrics.jsonl`, per-epoch evaluation JSON, checkpoints, provenance, or
TensorBoard events. Thus machine-readable truth and human presentation stay
separate.
...
Every run directory printed by `hm3d-semseg train` contains:

| Relative path | Contents |
|---|---|
| `metrics.jsonl` | Append-only step, epoch, development, and early-stopping records. Epoch numbers are zero-based. |
| `metrics_summary.json` | Initial/final/minimum training loss, histories, optimization summary, development best/history, and plot links. |
| `summary.json` | Run-level counts, device, step plan, parameter groups, checkpoint-selection value, and major artifact paths. |
| `resolved_config.yaml` | Exact merged scientific and host configuration used by the process. |
| `provenance.json` | Git, packages, hardware, model revision, datasets, selected sample IDs, and reproducibility settings. |
| `parameter_counts.json` | Trainable/total parameters and optimizer-group sizes/base rates/weight decay. |
| `plots/loss_and_learning_rate.png` | Robust step-bin median/10th--90th-percentile cross-entropy plus exact scheduled learning rates. |
| `plots/optimization_diagnostics.png` | Finite gradient norms, non-finite count, throughput, step duration, and peak CUDA memory. |
...
| `diagnostics/training_progress/qualitative/selection.json` | Prediction-independent fixed sample IDs, scenes, ground-truth class coverage, policy, and seed. |
| `diagnostics/training_progress/qualitative/{train,development}/contact_sheets/epoch_EEE.png` | Up to ten fixed rows: RGB, ground truth, prediction, correct/error map, and confidence. |
| `diagnostics/training_progress/qualitative/{train,development}/samples/` | RGB/ground truth stored once; per-epoch prediction, error, confidence, and sample metrics. |
| `report/index.html` | Primary static dashboard with sortable tables, warnings, plots, and qualitative epoch sliders. |
| `report/summary.md` | Compact text report suitable for review or a commit/experiment note. |
| `report/tables/*.csv` | Epoch, checkpoint, best-class, ObjectNav-six, best-scene, and largest-confusion tables. |
| `report/plots/*.png` | Generalization, calibration, class/scene evolution, ObjectNav, best confusion, and robust optimization plots. |
| `report/report_manifest.json` | Hashes and source paths proving which raw records produced the report. |

When a development dataset is configured, it also contains:
...
When `evaluate_train_subset: true`, as in tiny overfit:
...
These are memorization diagnostics on training images, not held-out evidence.
